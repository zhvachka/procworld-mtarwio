...
class "GeometryStruct" {
	...
	init = function(self,version)
		...
		self.version = version or EnumRWVersion.GTASA
...
class "Geometry" {	typeID = 0x0F,
	...
	mergeGeometry = function(self,target,clone)
		...
		local selfFaceIndex
		local selfMaterialCount = (self.materialList and self.materialList.struct and self.materialList.struct.materialCount) or 0
		...
		if not self.struct.bNative then
			...
			for i=1,#targetFaces do	--Copy faces
				selfFaces[i+selfFaceIndex] = {
					targetFaces[i][1]+selfVertexCount,
					targetFaces[i][2]+selfVertexCount,
					(targetFaces[i][3] or 0)+selfMaterialCount,
					targetFaces[i][4]+selfVertexCount,
				}
			end
		end
		--for i=1,self.morphTargetCount do	--morphTargetCount must be 1
		...
		local selfMatListIndex = selfMaterialCount
		for i=1,matListToMerge.struct.materialCount do
			local materialSlot = i + selfMaterialCount
			local materialIndex = matListToMerge.struct.materialIndices[i]
			if tonumber(materialIndex) and materialIndex >= 0 then
				materialIndex = materialIndex + selfMaterialCount
			end
			self.materialList.struct.materialIndices[materialSlot] = materialIndex
			self.materialList.materials[materialSlot] = matListToMerge.materials[i]
			if self.materialList.materials[materialSlot] then
				self.materialList.materials[materialSlot].parent = self.materialList
			end
		end
		...
		if selfExtension.binMeshPLG and targetExtension.binMeshPLG then
			if selfExtension.binMeshPLG.faceType == targetExtension.binMeshPLG.faceType then --FaceType should be the same
				...
				for i=1,targetBinMesh.materialSplitCount do
					--IndexCount, MaterialIndex, VertexIndexList
					...
					for vertexIndex=1,targetBinMesh.materialSplits[i][1] do
						selfBinMesh.materialSplits[matIndex][3][vertexIndex] = targetBinMesh.materialSplits[i][3][vertexIndex]+selfVertexCount	--Vertex Index
					end
				end
...
class "DFFIO" {
	...
	createClump = function(self,version)
		self.clumps = type(self.clumps) == "table" and self.clumps or {}
		local clump = Clump()
		clump.parent = self
